...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void streaming_service() {
        //todo: feel free to change code as you need
        Flux<Message> messageFlux = streamingService().flatMapMany(flux -> flux);
    ...
    public void i_am_rubber_you_are_glue() {
        //todo: feel free to change code as you need
        Flux<Integer> numbers1 = numberService1().concatWith(numberService2());
        Flux<Integer> numbers2 = Flux.concat(numberService1(), numberService2());

        //don't change below this line
        StepVerifier.create(numbers1)
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((first, others) -> {
            boolean isSpam = Objects.requireNonNull(first.get()).metaData.contains("spam");
            if (isSpam) return mailBoxSecondary();
            else return others;
        });
    ...
    public void acid_durability() {
        ...
        Flux<String> committedTasksIds = tasksToExecute()
                .flatMapSequential(task -> task.flatMap(taskId -> commitTask(taskId).thenReturn(taskId)));
    ...
    @Test
    public void cleanup() {
        // 실행 시 JVM 옵션 -XX:+AllowRedefinitionToAddDeleteMethods 추가해야 함
        BlockHound.install(); //don't change this line, blocking = cheating!

        //todo: feel free to change code as you need
        // Flux.using: sync cleanup
        // Flux.usingWhen: async handling
        Flux<String> stream = Flux.usingWhen(
                StreamingConnection.startStreaming(),
                resource -> resource,
                resource -> StreamingConnection.closeConnection()
        );
